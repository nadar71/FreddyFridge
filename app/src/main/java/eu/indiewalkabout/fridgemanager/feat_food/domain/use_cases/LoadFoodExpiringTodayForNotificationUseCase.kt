...
class LoadFoodExpiringTodayForNotificationUseCase @Inject constructor(
    private val repository: FridgeManagerRepository,
) {
    suspend operator fun invoke(dayBefore: Long?, dayAfter: Long?): List<FoodEntry> {
        return repository.loadFoodExpiringToday(dayBefore, dayAfter)
    }
}
