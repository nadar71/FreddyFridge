...
class ObserveAllFoodExpiringUseCase @Inject constructor(
    private val repository: FridgeManagerRepository,
    @ApplicationContext private val context: Context
) {
    operator fun invoke(date: Long?): Flow<DbResponse<List<FoodEntry>>> {
        return repository.observeAllFoodExpiring(date)
            .map<List<FoodEntry>, DbResponse<List<FoodEntry>>> { DbResponse.Success(it) }
            .catch { e ->
                Log.e(
                    "LoadExpiringFoodUseCase",
                    e.localizedMessage ?: context.getString(R.string.db_error_loading_expiring_food)
                )
                emit(
                    DbResponse.Error(
                        ErrorResponse(
