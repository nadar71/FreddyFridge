...
class ObserveFoodExpiringTodayUseCase @Inject constructor(
    private val repository: FridgeManagerRepository,
    @ApplicationContext private val context: Context
) {
    operator fun invoke(dayBefore: Long?, dayAfter: Long?): Flow<DbResponse<List<FoodEntry>>> {
        return repository.observeFoodExpiringToday(dayBefore, dayAfter)
            .map<List<FoodEntry>, DbResponse<List<FoodEntry>>> { DbResponse.Success(it) }
            .catch { e ->
                Log.e(
                    "LoadFoodExpiringTodayUseCase",
                    e.localizedMessage ?: context.getString(R.string.db_error_loading_expiring_today_food)
                )
                emit(
                    DbResponse.Error(
                        ErrorResponse(
