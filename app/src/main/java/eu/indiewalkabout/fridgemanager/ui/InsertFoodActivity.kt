...
class InsertFoodActivity : AppCompatActivity(), CalendarView.OnDateChangeListener, OnFABMenuSelectedListener {
    ...
    internal lateinit var foodEntryToChange: LiveData<FoodEntry>
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mAdView!!.loadAd(ConsentSDK.getAdRequest(this@InsertFoodActivity))
    ...
    private fun initViews() {
        ...
        val dateTodayNormalizedAtMidnight = DateUtility.getLocalMidnightFromNormalizedUtcDate(DateUtility.normalizedUtcMsForToday)
        // dateExpir_cv.setDate(dateTodayNormalizedAtMidnight + DateUtility.DAY_IN_MILLIS);
        dateExpir_cv!!.date = dateTodayNormalizedAtMidnight

        // saving editing test click
        save_btn!!.setOnClickListener { onSaveBtnClicked() }

        // speaker activation
        speakerBtn!!.setOnClickListener { startVoiceInput() }
        ...
        AppExecutors.instance!!.diskIO().execute {
            if (foodId == DEFAULT_ID) {     // save a new task
                ...
                if (datePicked != null) {
                    ...
                    foodEntry = FoodEntry(0, foodName, DateConverter.toDate(expiringDate_l)!!)
                }
    ...
    private fun populateUI(foodEntry: FoodEntry?) {
        ...
        dateExpir_cv!!.date = DateConverter.fromDate(foodEntry.expiringAt)!!
    }
    ...
    fun setDateExpir_cv(date: Date) {
        dateExpir_cv!!.date = DateConverter.fromDate(date)!!
    }
